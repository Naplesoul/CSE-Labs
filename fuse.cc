...
getattr(chfs_client::inum inum, struct stat &st)
{
    ...
    chfs_client::dirinfo dirinfo;
    chfs_client::fileinfo fileinfo;
    switch (chfs->gettype(inum))
    {
    case extent_protocol::T_DIR:
        ret = chfs->getdir(inum, dirinfo);
        if(ret != chfs_client::OK)
            return ret;
        st.st_mode = S_IFDIR | 0777;
        st.st_nlink = 2;
        st.st_atime = dirinfo.atime;
        st.st_mtime = dirinfo.mtime;
        st.st_ctime = dirinfo.ctime;
        printf("   getattr -> %lu %lu %lu\n", dirinfo.atime, dirinfo.mtime, dirinfo.ctime);
        break;
    
    case extent_protocol::T_FILE:
        ret = chfs->getfile(inum, fileinfo);
        if(ret != chfs_client::OK)
            return ret;
        st.st_mode = S_IFREG | 0666;
        st.st_nlink = 1;
        st.st_atime = fileinfo.atime;
        st.st_mtime = fileinfo.mtime;
        ...
        mode_t mode)
{
    ...
    struct stat st;
    chfs_client::status ret;
    chfs_client::inum inum_out;

    ret = chfs->mkdir(parent, name, mode, inum_out);
    switch (ret)
    {
    case chfs_client::OK:
        e.ino = inum_out;
        if (getattr(inum_out, st) != chfs_client::OK) {
            fuse_reply_err(req, ENOENT);
            return;
        }
        e.attr = st;
...
void
fuseserver_symlink(fuse_req_t req, const char *real_path, fuse_ino_t parent, const char *name)
{
    struct fuse_entry_param e;
    // In chfs, timeouts are always set to 0.0, and generations are always set to 0
    e.attr_timeout = 0.0;
    ...
    ret = chfs->symlink(real_path, parent, name, inum_out);
    switch (ret)
    {
    case chfs_client::OK:
        e.ino = inum_out;
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    if (chfs->readlink(ino, real_path) != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
    fuse_reply_readlink(req, real_path.c_str());    
}
